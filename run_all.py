...
Runs (in ./, writing to ./outputs):
  ...
  xcop_rnei_nsig.py              -> xcop_rnei_nsig.txt  (X-COP n_sigma=0.918)
  make_tab_regimes.py            -> tab_regimes.csv + tab_regimes_body.tex  (Table A.1)
  forward_lensing_delta_sigma.py -> forward_lensing_results.csv     (Appendix A.4, HMG)
  compare_fwd_models.py          -> outputs/compare_fwd_models.csv  (Appendix A.4, all models)

The last two scripts (Appendix A.4) use Abel quadrature and take ~5--30 min
combined on a modern machine.  Pre-computed CSV outputs are included in the
repository so the paper results can be checked without re-running them.
...
STEPS = [
    ...
    "forward_lensing_delta_sigma.py",  # Appendix A.4 (HMG): Abel FWD vs SIS; ~5 min
    "compare_fwd_models.py",       # Appendix A.4 (all models): HMG/MOND/CDM SIS+FWD chi2_nu; ~20-30 min
]
